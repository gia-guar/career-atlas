"""Threaded driver that runs scraping + skill_graph for the web UI.
...
from job_universe.web import progress
from job_universe.web.progress import ProgressEmitter

logger = logging.getLogger(__name__)

# src/job_universe/web/runner.py → parents[3] is the kedro project root.
...
class BuildJob:
    ...
    def _run(self) -> None:
        try:
            ...
            with KedroSession.create(project_path=PROJECT_ROOT) as session:
                session.run(pipeline_name="skill_graph")
            self.status = "done"
...
_GRAPH_JSON_PATH = project_data_dir() / "02_intermediate" / "skill_graph.json"
...
def status_flags() -> dict[str, Any]:
    return {
        ...
        "has_graph": _GRAPH_JSON_PATH.exists(),
    }
...
def lookup_postings_for_skill(
    ...
    skill_graph propagate without restarting the server.
    """
