"""FastAPI app that drives the SKILL-GRAPH pipelines from a browser."""
...
app = FastAPI(title="SKILL-GRAPH", version="0.3.0")
...
async def get_graph() -> JSONResponse:
    path = runner.paths()["graph"]
    if not path.exists():
        raise HTTPException(status_code=404, detail="skill_graph.json not yet built")
...
def run() -> None:
    """Console-script entry: ``job-universe-ui`` launches uvicorn."""
    import uvicorn

    uvicorn.run(
        "job_universe.web.app:app",
        host="127.0.0.1",
        port=8000,
        reload=False,
        log_level="info",
